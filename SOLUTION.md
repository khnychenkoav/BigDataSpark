...
В составе решения есть PostgreSQL для исходных данных и модели снежинка, Spark job для трансформаций, ClickHouse для обязательных отчетов, а также Cassandra, MongoDB, Neo4j и Valkey для дополнительных NoSQL-витрин
...
В PostgreSQL создается таблица фактов dw.fact_sales и набор измерений для клиентов, продавцов, магазинов, поставщиков, продуктов, стран и питомцев. Часть атрибутов вынесена в отдельные справочники, поэтому модель получается ближе к снежинке, но центральная аналитическая структура остается классической звездой вокруг факта продаж.

SparkSession ограничивает локальный параллелизм, число shuffle-partitions, память driver/executor и maxResultSize, чтобы лабораторный запуск не забирал все ресурсы машины.
